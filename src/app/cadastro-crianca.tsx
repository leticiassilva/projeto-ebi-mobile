...
import { View, TextInput, Text, Alert, ScrollView, TouchableOpacity, ActivityIndicator } from 'react-native';
...
export default function CadastrarCrianca() {
  ...
  const formatarTelefone = (texto: string) => {
    ...
    if (numeros.length > 0) {
      ...
      if (numeros.length > 7) {
        telefoneFormatado = numeros.replace(/^(\d{2})(\d{5})(\d{4}).*/, '($1) $2-$3');
      }
    }
  ...
  const formatarCPF = (texto: string) => {
    ...
    if (numeros.length > 9) {
      cpfFormatado = numeros.replace(/^(\d{3})(\d{3})(\d{3})(\d{2}).*/, '$1.$2.$3-$4');
    }
    return cpfFormatado;
  };
  ...
  const handleSubmit = async () => {
    if (!nomeCrianca || !idade || !dataNascimento || !responsavel || !telefoneResponsavel || !cpfResponsavel) {
